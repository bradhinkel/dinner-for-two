...
**Phases 0–2 + the restaurant-import pipeline are done and merged to `main` (PR #1). Ongoing import/curation work continues directly on `main`.** Day-to-day progress + decisions are also in agent memory (`MEMORY.md`).
...
**Catalog: 88 restaurants, 66 active** (19 → 32 → 78 → 88; 83 with dishes, ~2,533 dishes). 7 are `CLOSED_PERMANENTLY` (Places-flagged, auto-suppressed) and 15 are `curation:"hide"`; **retrieval serves the 66 active**. Data model carries `curation` (`use`/`hide`), `latitude`/`longitude`, `business_status`.
...
**Vision-OCR pass — built + proven, but low-yield here** (`extractMenuFromVision` in `extractMenu.ts`; `fetchMenu` captures vision sources). Sends image URLs / a base64 PDF document block to Sonnet 4.6 (OCRs both). Image URLs are downloaded to **base64 ourselves** (host-independent) with URL-source fallback — Anthropic's server 400s on hotlink-protected hosts. Menu images are read from the **page HTML source** (`page.content()`), NOT live `<img>` DOM, because lazy galleries (Squarespace) swap `currentSrc` to a placeholder and lazy-UNLOAD off-screen images. **Validated on Rob Roy (7 PNG scans → 42 beverages).** Verdict from the live re-run: image menus are RARE in this catalog — most vision triggers were hero/food photos (0 dishes), and the genuine image menus (Ethan Stowell sites, Flatstick) serve images even base64 can't fetch. Net vision win: Elliott Bay Brewery (23/12). The render fixes mattered more than vision: re-running `live` recovered ~8 new TEXT menus (Ben Paris, Ivar's, Le Pichet, Harvest Vine, Marination, San Fermo, Shiro's Sushi 66, …).

### ▶ NEXT STEPS (toward the Phase 3 "100 solid" gate — at 66 active now)

1. **Both big render levers are spent** (deep-follow via Six Seven; vision via Rob Roy). Remaining failures are: image menus on un-fetchable hosts, bot-blocked empty shells (`<200` chars to headless), and big sushi menus that overflow extract `max_tokens` (Momiji ×2, Sushi Zen, Tamarind Tree hit "extraction truncated" at 16000 — would need a higher cap or a chunked pass). All lower-value or harder than the ~34 rooms still short of 100.
2. To reach 100: keep mining the unworked viable pool with the improved renderer, OR raise the extract `max_tokens` (16000→24000) to recover the truncated big-menu rooms. `--url <menu-or-pdf>` remains the escape hatch for hostile sites (Daniel's Broiler 3-hop Shopify). `Cafe Munir` → drop (off-domain redirect).
